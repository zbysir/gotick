...
func NewStdRedisDelayedQueue(redis *redis.Client) *StdRedisDelayedQueue {
	return &StdRedisDelayedQueue{redis: redis, callbacks: make(map[string][]func(data string) error)}
}

func (r *StdRedisDelayedQueue) Publish(topic string, data string, delay time.Duration) error {
...
func (r *StdRedisDelayedQueue) Subscribe(topic string, h func(data string) error) {
